...
//   const { salonId, salonName, city } = route.params;
...
//   const phoneNumber = '+91 6302042304';
...
//         category: category
//       });
//       console.log("duration:", service.completion_time.split(':')[0]);
//       return acc;
//     }, {});
//   };
...
//     console.log("🛠 Full Selected Services:", updatedServices); // full details
//     console.log("🆔 Selected Service IDs:", updatedServices.map(s => s.service));
...
//       Alert.alert(
//         "Booking Successful",
//         `Your appointment for ${selectedServices.length} service(s) has been booked successfully.`,
//         [
//           {
//             text: "OK",
//             onPress: () => {
...
//       // For today's date, check if time is in the past
//       const now = new Date();
...
//       return slotTime < now;
//     }
//     return false; // For future dates, no time is in the past
//   };
...
//     styles.timeSlotText,
//     selectedTime === time && styles.selectedTimeSlotText,
//     isDisabled && { color: '#bbb' } // greyed out
//   ]}>
//     {formatTime(time)}
//   </Text>
// </TouchableOpacity>
...
// const SalonServices = ({ route, navigation }) => {
// const { salonId, salonName, city, SalonPhoneNumber, SalonLatitude, SalonLongitude } = route.params;  const { customer } = useAuth();
//   const customerId = customer?.data?.customer_id;
//   const [activeTab, setActiveTab] = useState('Male');
//   const [selectedServices, setSelectedServices] = useState([]);
//   const [selectedDate, setSelectedDate] = useState(new Date().toISOString().split('T')[0]);
...
// const SalonServices = ({ route, navigation }) => {
//   const { salonId, salonName, city, SalonPhoneNumber, SalonLatitude, SalonLongitude } = route.params;
...
//           specialty: stylist.user_role,
//           rating: '4.5',
//           profile_image: stylist.profile_image // Add profile picture if available
//         }));
...
//           style={styles.stylistImage}
//           onError={() => console.log(`Failed to load image for ${stylist.name}`)}
//         />
//       );
//     } else {
//       return (
...
const SalonServices = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // CORRECTED: Filter stylists for this salon with Active status
        const salonStylists = allUsers.filter(user => {
          const isStylist = user.user_role === 'Stylist' || user.user_role === 'Stylist/Admin';
          const belongsToSalon = user.salon == salonId;
          const isActive = user.status === 'Active';  // IMPORTANT: Check status
          
          return isStylist && belongsToSalon && isActive;
        }).map(stylist => ({
          id: stylist.id,
          name: stylist.full_name,
          specialty: stylist.user_role,
          rating: '4.5',
          status: stylist.status,  // Add status to the stylist object
          profile_image: stylist.profile_image ? `${BASE_URL}${stylist.profile_image}` : null
        }));

        console.log('👨‍💼 Stylists found (Active only):', {
          total: salonStylists.length,
          details: salonStylists.map(s => ({
            id: s.id,
            name: s.name,
            role: s.specialty,
            status: s.status
    ...
    fetchData();
  }, [salonId, activeTab, salonStatus]);  // Added salonStatus as dependency
  ...
  // Show message if no active stylists are available
  const renderStylistList = () => {
    if (stylists.length === 0) {
      return (
        <View style={styles.noStylistsContainer}>
          <FontAwesome name="user-times" size={40} color="#ccc" />
          <Text style={styles.noStylistsText}>No active stylists available</Text>
          <Text style={styles.noStylistsSubtext}>Please check back later</Text>
        </View>
      );
    }
      ...
      <FlatList
        ...
        renderItem={({ item: stylist }) => (
          ...
          >
            ...
            <View style={[
              styles.statusBadge,
              stylist.status === 'Active' ? styles.activeBadge : styles.inactiveBadge
            ]}>
              <Text style={styles.statusText}>{stylist.status}</Text>
            </View>
          </TouchableOpacity>
        )}
        contentContainerStyle={styles.stylistContainer}
      ...
      {salonStatus === "Offline" && (
        <View style={styles.offlineMessageContainer}>
          ...
          <Text style={styles.offlineMessageText}>
            This salon is currently offline. Services and stylists are not available.
          </Text>
        </View>
      )}

      {/* Only show content if salon is online */}
      {salonStatus !== "Offline" && (
        <>
          {/* Gender Tabs */}
          <View style={styles.tabsWrapper}>
