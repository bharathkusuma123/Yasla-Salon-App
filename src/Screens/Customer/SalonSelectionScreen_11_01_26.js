...
// Previous Working Code is in file Dated SalonSelectionScreen_10_01_26.js file 
//===================================================================================
// below code is after fixing below issue ---------
// timeslot issue Client Shared three videos and 
// Current Problem
//Service selected = 90 minutes
//Already booked slots = 9:00 AM and 9:30 AM
//In stylist timings, 8:30 AM is still shown as OPEN
//If customer books 8:30 AM, only 30 minutes is actually free
//Remaining time is blocked because 9:00 AM is already booked
...
const SalonSelectionScreen = ({ route }) => {
  ...
  // First, let's calculate total service duration from selectedServices
const calculateTotalServiceDuration = () => {
  if (!selectedServices || selectedServices.length === 0) return 0;
  
  // Assuming each service has a 'duration' field in minutes
  // If not, you might need to fetch service details to get duration
  let totalDuration = 0;
  selectedServices.forEach(service => {
    // If services have completion_time in seconds from CScreen1
    if (service.completion_time) {
      totalDuration += Math.round(parseFloat(service.completion_time) / 60);
    } else {
...
// Function to check if continuous time slots are available
const isContinuousTimeAvailable = (startTime, bookedSlots, totalDuration) => {
  // Convert start time to time slots index
  const timeSlots = timeSlotsFromCScreen1;
  ...
  for (let i = 0; i < slotsNeeded; i++) {
    ...
    if (bookedSlots.includes(currentTimeSlot)) {
      return false;
    }
  }
  ...
  // Check stylist availability for EXACT selected time slot
 // Modified checkStylistAvailabilityForExactTime function
const checkStylistAvailabilityForExactTime = async (stylistId) => {
  try {
    if (!selectedTimeSlot) return false;
    ...
    // Check continuous availability for the entire duration
    const isAvailable = isContinuousTimeAvailable(selectedTimeSlot, bookedSlots, totalDuration);

    console.log(`Stylist ${stylistId} continuous availability for ${selectedTimeSlot}:`, {
      totalDuration,
      isAvailable,
      bookedSlots
    });
...
// Also need to update checkSalonAvailability function
const checkSalonAvailability = async (salonId) => {
  try {
    const salonStylists = stylists.filter(stylist => stylist.salon === salonId);
    ...
    for (const stylist of salonStylists) {
      // Check if stylist has continuous availability for the selected time
      if (selectedTimeSlot) {
        const response = await axios.get(
          `https://yaslaservice.com:81/appointments/stylist/${stylist.id}/?start_date=${selectedDate}`
        );
...
// Modified fetchAvailableTimeSlots function
const fetchAvailableTimeSlots = async () => {
  if (!selectedStylist || !selectedDate) return;
  ...
  try {
    ...
    // Filter available slots based on continuous availability
    const available = timeSlotsFromCScreen1.filter(time => 
      isContinuousTimeAvailable(time, bookedSlots, totalDuration) && !isTimeSlotInPast(time)
    );
...
// In renderTimeSlots function, update the time slot display logic
const timeSlotsToShow = showAllTimeSlots ? timeSlotsFromCScreen1 : availableSlots;

// For each time slot, check if it's actually available for the full duration
// when showing all time slots
{timeSlotsToShow.map((time, index) => {
  const isPastTime = isTimeSlotInPast(time);
  
  // For showAllTimeSlots mode, check if this slot has continuous availability
  let isAvailable = true;
  if (showAllTimeSlots) {
    const totalDuration = calculateTotalServiceDuration();
    isAvailable = isContinuousTimeAvailable(time, bookedTimeSlots, totalDuration);
  } else {
    // For filtered mode, it's already filtered
  ...
  return (
    ...
    >
      ...
      >
        {formatTime(time)}
        {!isAvailable && showAllTimeSlots && (
          <Text style={{ fontSize: 10, color: '#999' }}> \n(Not enough time)</Text>
        )}
      </Text>
    </TouchableOpacity>
  );
